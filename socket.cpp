...
Csocket::Csocket(CWnd* parent)
{
	...
	m_alive  = false;
	m_flow   = flowS;
	m_lock   = std::make_unique<CSingleLock>(&m_event);
}
...
void Csocket::OnConnect(int nErrorCode) 
{
	...
	{
		m_alive = true;
		m_parent->SendMessage(WM_USER +12, MAKEWPARAM(sm_CONNECT, 0));
	}
...
{
	m_alive = false;
	m_parent->PostMessage(WM_USER + 12, MAKEWPARAM(sm_CLOSE, 0));	
...
BOOL Csocket::Open(CString ips, int port)
{
	...
	CString slog;
	slog.Format("socket create failed %d", GetLastError());
	AfxMessageBox(slog);
	return FALSE;
}
...
BOOL Csocket::Write(char* sndB, int sndL)
{
	...
	for (int ii = 0, nWritten = 0; ;)
	{
		nWritten = Send(&sndB[ii], sndL);
